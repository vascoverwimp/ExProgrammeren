...
def main() -> None:
    ...
    print(f"Run BurgersPINN/plot.py --situation {args.situation} --viscosity {args.viscosity} to generate plots")
...
def evaluate_model(situation: str = BurgerConfig.situation, v: float = BurgerConfig.v) -> tuple[float, float, float]:
    ...
    config = raw["config"]
    data = raw["data"]
    ckpt_ml = out_dir / f"{config.situation}_{config.v:.1e}_{config.suffix_ckpt_ml}"

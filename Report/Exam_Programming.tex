...
In this section I will demonstrate the code for the three different damping cases, and discuss the results and effects of different factors. In the first part of this section, we will use the following hyperparameters do predict the solution for the overdamped, critically damped and underdamped cases.:
...
The previous three demonstration examples are a good illustration of the capabilities of the inverse model, but are not enough to make well-reasoned and statistically backed conlcusions about the model acuraccy. For this, we need a more quantifiable way to gauge the accuracy of the model.\\
...
\section{Code demonstration}
In this section, we will demonstrate PINNs for three different initial conditions (Gauss, N-wave and Step-up functions), and three different values for the viscosity ($\nu=0.1,\ 0.05,\ 0.01$). Unless specified otherwise, we will use the following hyperparameters:
...
The code is implemented in such a way that we can include training data in the training loop by modifying the Config class. We want to compare the results with and without training data. The predicted solution where data is included in the training is shown below:
\begin{figure}[H]
    \centering
...
With the inverse problem in mind, it is wise to look at the effect of different hyperparameters on the model predictions to determine an optimal hyperparameter set. Like in the previous chapter about the damped harmonic oscillator, I will discuss some of the most important factors and then apply a Tree-Stuctured Parzen Estimator using \verb|optuna|.
...
Finally, an algorithmic search of the best hyperparameters can be useful to gain further insights to decide on an optimal parameter set for the inverse problem. For this, we have to generate noisy observation points and add the data loss term to the total loss function, so it becomes:
...
We will now attempt to solve the inverse problem for gaussian initial conditions. The implementation is very similar to the inverse problem for the damped harmonic oscillator. We implement an \verb|InverseFCNet| with a $\hat\nu$ parameter, and \verb|loss_physics_inverse| function that will be used to calculate the loss in the training loop.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/backward_demo_gauss_highnu/pred_solution.png}
    \caption{Predicted solutions at four different times for the backward problem with $\nu=0.4028$ and $\hat\nu=0.3886$.}
    \label{fig:backward_demo_gauss_highnu/pred_solution}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/backward_demo_gauss_highnu/summary.png}
    \caption{Summary plot for the backward problem with $\nu=0.4028$ and $\hat\nu=0.3886$.}
    \label{fig:backward_demo_gauss_highnu/summary}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_BE/backward_demo_gauss_highnu/parameter_convergence.png}
    \caption{Parameter convergence for the backward problem  with $\nu=0.4028$ and $\hat\nu=0.3886$.}
    \label{fig:backward_demo_gauss_highnu/parameter_convergence}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/backward_demo_gauss_lownu/pred_solution.png}
    \caption{Predicted solutions at four different times for the backward problem with $\nu=0.0068$ and $\hat\nu=0.0179$.}
    \label{fig:backward_demo_gauss_lownu/pred_solution}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/backward_demo_gauss_lownu/summary.png}
    \caption{Summary plot for the backward problem with $\nu=0.0068$ and $\hat\nu=0.0179$}
    \label{fig:backward_demo_gauss_lownu/summary}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_BE/backward_demo_gauss_lownu/parameter_convergence.png}
    \caption{Parameter convergence for the backward problem with $\nu=0.0068$ and $\hat\nu=0.0179$}
    \label{fig:backward_demo_gauss_lownu/parameter_convergence}
\end{figure}
...
In order to make a better-reasoned and statistically backed conlcusions, I want to perform a large amount of parameter estimations. In order to make this process more efficient, I have chosen to reduce the complexity of the model to 6 layers of 64 nodes, which goes at the cost of the estimation accuracy, However, in this way I have performed a total of $767$ model training where the viscosity is estimated, which allows for statistical treatment of the data. The viscosity is randomised for every run, with a $50\%$ chance of a low viscosity ($0.005-0.05$) and a $50\%$ chance of a high viscosity ($0.05-0.5$). Figure \ref{fig:nu_estimation_2/scatter_Gauss_None} shows the true-predicted $\nu$ pairs in a scatter plot.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/nu_estimation_2/scatter_Gauss_None.png}
    \caption{Scatter plot of 767 $(\nu,\ \hat\nu)$ pairs for gaussian initial conditions.}
    \label{fig:nu_estimation_2/scatter_Gauss_None}
\end{figure}

We can see that in general, we are able to extract the viscosity with relative good accuracy. It is also interesting to split the dataset up into high and low viscosity, as shown on Figures \ref{fig:nu_estimation_2/scatter_Gauss_high} and \ref{fig:nu_estimation_2/scatter_Gauss_low}. 

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/nu_estimation_2/scatter_Gauss_high.png}
        \caption{High $\nu$.}
        \label{fig:nu_estimation_2/scatter_Gauss_high}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/nu_estimation_2/scatter_Gauss_low.png}
        \caption{Low $\nu$.}
        \label{fig:nu_estimation_2/scatter_Gauss_low}
    \end{subfigure}
    \caption{Scatter plots of $(\nu,\ \hat\nu)$ pairs for gaussian initial conditions.}
    \label{fig:nu_estimation_2/scatter_Gauss}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_BE/backward_demo_gauss_veryhighnu/parameter_convergence.png}
    \caption{Parameter convergence for a backward problem with $\nu=0.4500$ and $\hat\nu=0.3930$}
    \label{fig:backward_demo_gauss_veryhighnu/parameter_convergence}
\end{figure}

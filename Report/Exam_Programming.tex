...
Physics-Informed Neural Networks (PINNs), first introduced by M. raissi, P. Perdikaris abd G.E. Karniadakis in 2019 \cite{RAISSI2019686}, are a class of machine learning models which augment standard neural network training with prior knowledge of the physical laws governing a system. Rather than learning purely from data, a PINN is simultaneously trained to minimise the residual of a governing differential equation at a separate set of collocation points, enforcing consistency with the underlying physics throughout the domain. This makes PINNs particularly appealing in settings where observational data is scarce and/or noisy: these conditions are very common in scientific and engineering applications. There are many different examples in literature of different kinds of PINNs used to solve nonlineair ODEs and PDEs.\\

This report investigates the application of PINNs to two problems in mathematical physics. The first is the damped harmonic oscillator, a second-order ODE that describes oscillatory motion with energy dissipation (friction). The second is the one-dimensional viscid Bateman-Burger's equation, a nonlinear PDE which captures the competing effects of advection and diffusion, and which exhibits the physically interesting phenomenon of shock formation and rarefaction waves. This equation is also cald the \textit{poor man's} Navier-Stokes equation, as it treats the same concepts but is lower-dimensional and only takes diffusion and convection terms into account, without pressure or external forces.\\

For each problem, the report proceeds as follows. The governing equations and their analytic or numerical solutions are introduced to establish a ground truth. A PINN implementation is then developed and analysed in detail, covering the structure of the loss function, network architecture, training procedure, memory usage and and hyperparameter sensitivity. Improvements over a provided baseline example are proposed and justified, including randomised training data, early stopping, and systematic hyperparameter optimisation using the Optuna library. Finally, the inverse problem is tackled for both systems: extracting unknown physical parameters (the damping ratio $\zeta$ and natural frequency $\omega_0$ for the oscillator, and the kinematic viscosity $\nu$ for Burger's equation) from noisy observational data.\\
...
This section is made using the \verb|pinn_profiling.py| script. Before invertigating the prediction performance, I will profile the cde and display the memory usage and training time for my local machine. All models discussed in this report were trained in a MEDION Defender P50 laptop. The specs for this laptop are shown below:

\begin{table}[H]
\centering
\caption{System Specifications}\label{tab:system_specs}
\begin{tabular}{llc}
\toprule
\textbf{Component} & \textbf{Description} & \textbf{Value} \\
\midrule
\texttt{CPU}        & Processor                   & Intel Core i7-13620H (13th Gen) × 16    \\
\texttt{RAM}        & System memory               & 32.0 GiB                                \\
\texttt{GPU}        & Discrete graphics           & NVIDIA GeForce RTX 4060 Laptop          \\
\texttt{VRAM}       & GPU memory                  & 8.0 GiB                                 \\
\texttt{iGPU}       & Integrated graphics         & Intel Graphics (RPL-P)                  \\
\texttt{Storage}    & Disk capacity               & 2.0 TB                                  \\
\texttt{OS}         & Operating system            & Ubuntu 24.04.4 LTS (64-bit)             \\
\texttt{Kernel}     & Kernel version              & Linux 6.17.0-29-generic                 \\
\texttt{CUDA}       & CUDA version                & 12.2                                    \\
\texttt{Driver}     & NVIDIA driver version       & 535.309.01                              \\
\bottomrule
\end{tabular}
\end{table}

We will run the training for the underdamped case again, and profile the following metrics for data generation, model training and plot generation:
\begin{itemize}
    \item The wall time, calculated using the \verb|time| module.
    \item The RAM usage over time, using the \verb|memory_profiler| library.
    \item The GPU VRAM usage over time, using the \verb|torch| and \verb|threading| libraries.
\end{itemize}
This gives the following runtimes:

\begin{table}[H]
\centering
\caption{Wall times}\label{tab:wall_times_DHO}
\begin{tabular}{lc}
\toprule
\textbf{Process} & \textbf{Time ([s])}\\
\midrule
Data generation        & $0.0$                        \\
Model training         & $16.2$                       \\
Plot generation        & $2.9$                        \\
...
\end{tabular}
\end{table}
The training finished after 4550 epochs, which results in a time per epoch of $3.56$~ms. Since the data generation happens almost instantly, it is not useful to track the memory usage for that process. The RAM and VRAM usage as a function of time\footnote{Note that the process takes longer than shown in Table \ref{tab:system_specs}. This is because of the overhead of the memory profiler.} for the model training is displayed below:

\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_DHO/pinn_profiling/mem_train.png}
        \caption{RAM usage as a function of time when training the model.}
        \label{fig:pinn_profiling/mem_train}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_DHO/pinn_profiling/mem_train_gpu.png}
        \caption{VRAM usage as a function of time when training the model.}
        \label{fig:pinn_profiling/mem_train_gpu}
    \end{subfigure}
    \caption{Memory profiling during model training.}
    \label{fig:pinn_profiling_train}
\end{figure}
We can see that the RAM usage is a stepwise function of time. This is likely a result of the epoch snapshots which will be used later for plotting. After a specific user-defined amount of epochs, the model weights will be saved to a dictionary \verb|snapshots|. Furthermore, the loss terms are saved after every 50 epochs, which will also contribute to the memory usage. Only at the end of the training loop will the saved variables be saved to a file in the output folder, and the RAM can be cleared again.\\

On the VRAM plot, we see that the usage seems to fluctuates a lot between $0$~mB and $9$~mB. These fluctuations are likely sampling artefacts because the model can be doing different tasks: updating the weights, performing a forward pass, saving outputs to a dictionary, displaying text on the screen etc. During all of these tasks, the VRAM usage will be different. The peak value is around $9$~mB, which is very small compared to the total available VRAM. This model is still relatively small (16833 parameters), which takes up little VRAM space. For larger models, like for the Burger's equation, we would expect more VRAM usage since the models will have to be more complex, with more parameters.\\

The RAM and VRAM usage for the plot generation is displayed below:
\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_DHO/pinn_profiling/mem_plots.png}
        \caption{RAM usage as a function of time when generating plots.}
        \label{fig:pinn_profiling/mem_plot}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_DHO/pinn_profiling/mem_plots_gpu.png}
        \caption{VRAM usage as a function of time when generating plots.}
        \label{fig:pinn_profiling/mem_plot_gpu}
    \end{subfigure}
    \caption{Memory profiling during model training.}
    \label{fig:pinn_profiling_plot}
\end{figure}

We see that the VRAM usage is at zero, which makes sense as the predictions are made from the CPU, not the GPU. The RAM usage is another stepwise function, likely increasing every time a new plot is made. The RAM usage is quite high, with peak valuesa at $120$~mD. This learns us that it is wise to close the plots after saving with \verb|plt.close|, in order to not waste RAM on plots that have already been saved.

\subsection{Comparison between blind and physics informed extrapolation}\label{ch:extrap_DHO}
This section is made using the \verb|pinn_blind_extrap.py| script. Before tuning the hyperparameters or attempting to solve the inverse problem, I want to study the effect of some of the loss terms. Firstly, in all previous simulations, the collocation points were allowed to be chosen in the extrapolated domain. This makes the extrapolation not fully blind, but physics-informed. In this experiment, the collocation points will only be chosen in the training domain, and no other hyperparameters will be changed.. This results in an actually blind test, as shown in Figure \ref{fig:pinn_blindextrap/summary}
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Scatter plot of 845 true and predicted $\zeta$ pairs.}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Scatter plot of 845 true and predicted $\omega_0$ pairs.}
...
\begin{figure}[H]
    ...
    \caption{Parameter space of 845 true $(\zeta, \omega_0)$ pairs and the $RMSE$ compared to the predicted values.}
...
We will now discuss the memory usage and the effect of different parameters for the trained models. Since the gaussian initial conditions gave the most accurate results and profuce a shockwave, I will stick to this inital condition with $\nu=0.05$ for the rest of the report.\\

\subsection{Profiling}
Just like for the damped harmonic oscillator, we will profile the runtime, RAM and VRAM usage for the data generation, model training, and plot generation. We would expect the runtimes to be longer and memory usage to be higher since the Burgers equation model is more complex than the damped harmonic oscillator model. Timing the different tasks gives the following runtimes:

\begin{table}[H]
\centering
\caption{Wall times}\label{tab:wall_times_BE}
\begin{tabular}{lc}
\toprule
\textbf{Process} & \textbf{Time ([s])}\\
\midrule
Data generation        & $15.0$                      \\
Model training         & $88.8$                       \\
Plot generation        & $24.5$                        \\
\bottomrule
\end{tabular}
\end{table}
The training finished after 7200 epochs, which results in a time per epoch of $12.33$~ms. This is noticably longer than the damped harmonic oscillator epoch time, which is logical as there are many more parameters to tune. We can also see that the data generation is no longer instantaneous, as we are not implementing a closed form solution for the ground truth; we are numerically computing the Cole-Hopf transformation. Because of this, we can also look at the memory usae during data generation. The RAM and VRAM usage as a function of time is displayed below:
\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/pinn_profiling/mem_data.png}
        \caption{RAM usage as a function of time when generating ground truth data.}
        \label{fig:pinn_profiling/mem_data_BE}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/pinn_profiling/mem_data_gpu.png}
        \caption{VRAM usage as a function of time when generating ground truth data.}
        \label{fig:pinn_profiling/mem_data_gpu_BE}
    \end{subfigure}
    \caption{Memory profiling during model training.}
    \label{fig:pinn_profiling_data_BE}
\end{figure}
Since the data generation occurs entirely on the CPU, the VRAM usage is zero everywhere. The RAM usage seems to be constant at around $10$~mB. This is due to the dense grid of ground truth Cole-Hopf grid points: at the beginning of the data generation, a dense $1000\times1000$ array is created which will be used to store the ground-truth Cole-Hopf calculated values. This array is a lot larger than all other generated variables; the second largest object is the $300\times300$ grid of validation points. Only the very first increase in memory usage due to the Cole-Hopf calculation is visible on the plot, every other generated object is negligent.\\

The RAM and VRAM usage as a function of time for the model training is displayed below:
\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/pinn_profiling/mem_train.png}
        \caption{RAM usage as a function of time when training the model.}
        \label{fig:pinn_profiling/mem_train_BE}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/pinn_profiling/mem_train_gpu.png}
        \caption{VRAM usage as a function of time when training the model.}
        \label{fig:pinn_profiling/mem_train_gpu_BE}
    \end{subfigure}
    \caption{Memory profiling during model training.}
    \label{fig:pinn_profiling_train_BE}
\end{figure}
Yet again, we see a step-wise RAM usage as snapshots and losses are logged. The VRAM plot also looks similar to the previous plot for the damped harmonic oscillator. Here it is even more clear that different tasks during the training loop cause different VRAM usages. The peak VRAM is a lot higher here, since there are many more parameters (65473 to be exact) to be optimized. This causes the peak value to be $\sim70$~mB compared to the $\sim8$~mB for the previous task.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/pinn_profiling/mem_plots.png}
        \caption{RAM usage as a function of time when generating plots.}
        \label{fig:pinn_profiling/mem_plot_BE}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_BE/pinn_profiling/mem_plots_gpu.png}
        \caption{VRAM usage as a function of time when generating plots.}
        \label{fig:pinn_profiling/mem_plot_gpu_BE}
    \end{subfigure}
    \caption{Memory profiling during model training.}
    \label{fig:pinn_profiling_plot_BE}
\end{figure}

The GPU isage is yet again at zero since the predictions are happening on the CPU. Just like for the damped harmonic oscillator, the RAM usage changes in steps as different plots are made. The usage stays at zero for a long time as the ground truth is being regenerated. The memory used for this task is likely not detected as the memory profiler only profiles the plot generation itself and the data generation falls outside of the profiling scope. On the plot, we can see that the memory usage is not strictly increasing like for the previous task. This can be because more plots are being made, and variables can be identically named across functions. This leads to some reference counters hitting zero, causing the garbage collector to free up this space in the memory while other plots are still being made. The large spike in the plot is likely caused by the summary figure, where 6 different panels need to be plotted, including 2D images of the predicted solution.
...
For the damped harmonic oscillator, PINNs were shown to accurately reconstruct the analytic solution across all three damping regimes (overdamped, critically damped, and underdamped) using fifteen noisy observation points per epoch, supplemented by the physics residual and initial condition constraints. We also showed that the PINN is capable of solving the ODE without the inclusion of any training points. The improvements made to the baseline implementation, including a more structured file system, randomised training data and collocation points, early stopping based on a validation loss derived from the analytic ground truth, and systematic hyperparameter optimisation via a Tree-Structured Parzen Estimator, resulted in a more robust, modular and reproducible training process. The extrapolation experiments highlighted a key strength of PINNs: when the physics residual is enforced in the extrapolated domain, the model produces physically consistent predictions beyond the training window, outperforming a traditional neural network which lacks this constraint.\\

The inverse problem for the damped harmonic oscillator demonstrated that the parameters $\zeta$ and $\omega_0$ can be reliably extracted from noisy data. A statistical analysis over 845 randomised parameter combinations revealed that estimation accuracy degrades in the overdamped regime, where solutions are less sensitive to variations in the parameters.\\

For Burger's equation, a ground truth was generated using the Cole-Hopf transformation, which converts the viscid equation into the heat equation and allows for an solution via numerical convolution. PINNs were then trained across three initial conditions (Gaussian, N-wave, and step-up) and three values of the kinematic viscosity without the inclusion of training data. The results confirmed that solutions with steeper gradients, corresponding to lower viscosity and shock-forming initial conditions, are inherently more difficult to fit with smooth $\tanh$ activation functions. In such cases, increased model complexity and adjusted loss weights were necessary, and demonstrated substantial improvements. The addition of a boundary condition loss term was found to have little practical impact for the Gaussian initial condition, but represents good practice when the boundaries interact more strongly with the interior dynamics.\\

The inverse problem for Burger's equation showed that the viscosity $\nu$ can be estimated from noisy data with reasonable accuracy. A statistical analysis over 767 training runs revealed a systematic bias: the model overestimates $\nu$ for low-viscosity solutions, because the smooth network output underestimates sharp spatial gradients, effectively attributing the mismatch to a higher viscosity. Conversely, the model underestimates $\nu$ for high-viscosity solutions, where the viscous term contributes less to the total loss and the inverse parameter gradient becomes correspondingly small.\\

Taken together, the results confirm that PINNs are a powerful and flexible tool for solving both forward and inverse problems governed by differential equations. Their ability to incorporate physical constraints or information directly into the training loop makes them particularly well-suited to data-scarce regimes, and their modular implementation makes them straightforward to adapt to new equations and problem settings.

...
BurgerConfig before anything else runs.  The script:
  ...
  3. Trains PINN models       (data + physics + IC loss).
  4. Saves the best checkpoint after every improvement so a crash
  never loses more than one log_every interval.
  5. Applies early stopping based on validation MSE.
  6. Recovers the learned parameter (viscosity).
...
from BurgersReversePINN.model import BurgerConfig, FCNet, Predictor
from BurgersReversePINN.Burger_PDE import BurgersSolver
...
def analytic(x: np.ndarray, t: np.ndarray, solver: BurgersSolver) -> np.ndarray:
    ...
    Args:
        ...
        cfg: BurgerConfig instance with problem parameters.
...
def loss_physics(
    ...
    of the PDE with learned viscosity: ∂u/∂t + u·∂u/∂x - v_hat·∂²u/∂x² = 0.
    ...
    *ckpt_path* — even if the run is interrupted afterwards the best weights
    ...
    for epoch in range(1, cfg.n_epochs + 1):
        ...
        if use_physics:
            ...
            l_phys = loss_physics(model, x_col_t, t_col_t)
...
def parse_args() -> argparse.Namespace:
    """
    Parse command-line arguments for training the Burgers equation reverse PINN.

    Defines arguments across nine groups — physical parameters, initial
    conditions, time domain, space domain, data, PINN loss weights,
    architecture, optimiser, training loop, early stopping, and output —
    with defaults drawn from ``BurgerConfig``. Includes ``--v_init`` for
    supplying an initial viscosity guess to the PINN optimiser, and
    ``--lr_param`` for setting a separate learning rate for learned physical
    parameters.

    Returns:
        argparse.Namespace: Parsed arguments, with one attribute per flag
            (e.g. ``args.viscosity``, ``args.v_init``, ``args.lr_param``).
    """
...
def main() -> None:
    """
    Entry point for training and evaluating the Burgers equation reverse PINN
    models.
    ...
    3. Summarises the run configuration (true viscosity, initial viscosity
       guess, scenario, observation count, noise level, collocation grid)
       to stdout.
    4. Trains both models (PINN with extended physics and PINN blind) by
       delegating to :func:`train_and_save_both`.
    5. Evaluates and compares both models via :func:`evaluate_model`.
    ...
    print(
        f"Run BurgersReversePINN/plot.py --situation {args.situation}"
...
def evaluate_model(situation: str = BurgerConfig.situation,
                   v: float = BurgerConfig.v) -> tuple[float, float]:
    """
    Evaluate and compare the PINN (extended physics) and PINN (blind) models
    against ground-truth data, then print a side-by-side results summary.

    Loads the best-validation checkpoints for both models from disk, runs
    inference over the full spatiotemporal grid, and computes RMSE on the
    training interval, RMSE on the extrapolation interval, physics residual,
    and the inferred viscosity parameter (nu_hat) for each model.
    ...
    Returns:
        tuple[float, float]: A ``(nu_hat_blind, nu_hat_ext_phys)`` pair
            containing the viscosity estimates inferred by the blind PINN
            and the extended-physics PINN, respectively.

    Side effects:
        Prints a formatted results table to stdout comparing both models
        across all metrics.
    """
...
def evaluate_blind(situation: str = BurgerConfig.situation,
    ...
    inference over the full spatiotemporal grid, and computes RMSE on the
    training interval, RMSE on the extrapolation interval, physics residual,
    and the inferred viscosity parameter (nu_hat).
    ...
    Returns:
        tuple[float, float]: A ``(rmse_pinn_blind_train, nu_hat_blind)`` pair
            containing the training-interval RMSE and the inferred viscosity
            estimate for the blind PINN.

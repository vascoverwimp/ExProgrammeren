...
class DamperConfig:
    ...
    # ── Data ─────────────────────────────────────────────────────────────────
    n_obs:       int   = 15    # total noisy observations (before split)
    val_fraction: float = 0.2  # fraction of observations held out for val
    sigma:       float = 0.05  # measurement noise std dev
    n_col:       int   = 200   # collocation points (physics residual)
    seed:        int   = 42    # global RNG seed

    # ── PINN loss weights ─────────────────────────────────────────────────────
    lambda_phys: float = 1e-2  # physics-residual weight
    ...
    # ── Output paths ──────────────────────────────────────────────────────────
    out_dir:    str = "./VascoVersionPINNDamper/Output"                  # directory for all saved files
    results_pt: str = "training_results.pt"  # torch.save bundle (relative)
    ckpt_ml:    str = "best_ml.pt"           # best-so-far ML checkpoint

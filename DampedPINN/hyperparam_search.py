...
def search_learning_rate():
    ...
    learning_rates =np.logspace(-3.5, -1.5, num=9) # Search over a range of learning rates from 10^-3.5 to 10^-1.5: [3.16e-4, 5.62e-4, 1e-3, 1.78e-3, 3.16e-3, 5.62e-3, 1e-2, 1.78e-2, 3.16e-2]
...
def search_num_collocations():
    num_collocations = np.linspace(10, 500, num=50, dtype=int)  # Search over a range of numbers of collocation points
    plot_RMSE = []  # To store RMSE values for plotting later
    device = get_device()
    default_cfg = DamperConfig()
    out_dir = Path(default_cfg.out_dir)

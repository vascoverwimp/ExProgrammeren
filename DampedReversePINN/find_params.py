...
def damper_predictions():
    ...
    for damping in dampings:
        for stiffness in stiffnesses:
            ...
            true_zeta = cfg.zeta
            location = Path(
                f"{cfg.out_dir}/w0{cfg.omega_0:.1e}_zeta{cfg.zeta:.1e}_{cfg.suffix_results_pt}")

            if not location.exists():
                train_and_save_both(**kwargs)

            w0_hat_blind, zeta_hat_blind, w0_hat_ext_phys, zeta_hat_ext_phys = evaluate_model(
                true_w0, true_zeta)
    ...
    zeta_true = np.array([true_zeta for _, _, _, true_zeta, _, _,
    ...
    axes[0].set_xticks(range(len(stiffnesses)))
    axes[0].set_xticklabels([f'{s:.1f}' for s in stiffnesses], rotation=45)
    axes[0].set_yticks(range(len(dampings)))
    axes[0].set_yticklabels([f'{d:.1f}' for d in dampings])
    plt.colorbar(im0, ax=axes[0])

    im1 = axes[1].imshow(zeta_blind, cmap='viridis',
                         aspect='auto', origin='lower', vmin=vmin, vmax=vmax)
    axes[1].set_xlabel('Stiffness')
    axes[1].set_ylabel('Damping')
    axes[1].set_title('PINN Blind Zeta')
    axes[1].set_xticks(range(len(stiffnesses)))
    axes[1].set_xticklabels([f'{s:.1f}' for s in stiffnesses], rotation=45)
    axes[1].set_yticks(range(len(dampings)))
    axes[1].set_yticklabels([f'{d:.1f}' for d in dampings])
    plt.colorbar(im1, ax=axes[1])

    im2 = axes[2].imshow(zeta_ext_phys, cmap='viridis',
                         aspect='auto', origin='lower', vmin=vmin, vmax=vmax)
    axes[2].set_xlabel('Stiffness')
    axes[2].set_ylabel('Damping')
    axes[2].set_title('PINN Ext Phys Zeta')
    axes[2].set_xticks(range(len(stiffnesses)))
    axes[2].set_xticklabels([f'{s:.1f}' for s in stiffnesses], rotation=45)
    axes[2].set_yticks(range(len(dampings)))
    axes[2].set_yticklabels([f'{d:.1f}' for d in dampings])
    ...
    omega0_true = np.array([true_w0 for _, _, true_w0, _, _, _, _, _ in plotting]).reshape(
        len(dampings), len(stiffnesses))
    ...
    im1 = axes[1].imshow(omega0_blind, cmap='viridis',
                         aspect='auto', origin='lower', vmin=vmin_w, vmax=vmax_w)
    axes[1].set_xlabel('Stiffness')
    axes[1].set_ylabel('Damping')
    ...
    im2 = axes[2].imshow(omega0_ext_phys, cmap='viridis',
                         aspect='auto', origin='lower', vmin=vmin_w, vmax=vmax_w)
    axes[2].set_xlabel('Stiffness')
    axes[2].set_ylabel('Damping')
